...
5. 困难：初始化线上850w+的数据时，1.发现数据执行的很慢，2.发现有一个warn级别的日志在无限打印
   ...
   3. warn级别的日志一直在打印是因为自己的网关操作，但是按道理讲定时任务的执行应该是找到容器中的bean去执行的方法，但是这个地方竟然进了网关，很奇怪，需要排查
   4. warn级别的日志疯狂重复打印的原因是 mybatis-plus 的一个自动注入的插件，在每次执行insert update的时候会自动填充当前登陆人和创建时间字段，当前登陆人这个字段是从上下文的登录信息中获取的，因为没有走网关所有获取不到信息，加上上面说的mybatis-plus的批量插入的底层是类似for循环的无限操作，导致日志疯狂刷
   5. 解决方案:定位到具体出现的地方，找相对应的负责人，商量如何解决，最终解决方案是将warn级别的日志取消
